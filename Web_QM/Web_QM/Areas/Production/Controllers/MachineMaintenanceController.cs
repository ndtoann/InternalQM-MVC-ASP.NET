...
namespace Web_QM.Areas.Production.Controllers
{
    ...
    public class MachineMaintenanceController : Controller
    {
        ...
        [Authorize(Policy = "EditMachineMaintenance")]
        [HttpPost]
        public async Task<IActionResult> Confirm([FromBody] List<long> ids)
        {
            if (ids == null || !ids.Any())
            {
                return BadRequest(new { message = "Vui lòng chọn ít nhất một mục." });
            }

            try
            {
                var items = await _context.MachineMaintenances.Where(m => ids.Contains(m.Id)).ToListAsync();
            ...
            {
                return StatusCode(500, new { message = $"Đã xảy ra lỗi khi duyệt: {ex.Message}" });
            }
        }
