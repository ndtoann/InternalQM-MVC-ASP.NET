...
namespace Web_QM.Areas.HR.Controllers
{
    ...
    public class ExamTrainingController : Controller
    {
        ...
        [Authorize(Policy = "AddExam")]
        [HttpPost]
        public async Task<IActionResult> Add(ExamTraining exam, IFormFile essayPdf)
        {
        ...
        [Authorize(Policy = "EditExam")]
        [HttpPost]
        public async Task<IActionResult> Edit(long id, ExamTraining exam, IFormFile essayPdf)
        {
            if (id != exam.Id)
            {
                return NotFound();
